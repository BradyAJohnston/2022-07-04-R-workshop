...
We can use the code chunks ``` `` ``` and start it with `` `r ` `` to tell `knitr` that we want it to be treated as code, `` `r our_result` `` will be: `r our_result`.
...
For example, today's date is `r Sys.Date()`, will be rendered as: today's date is `r Sys.Date()`. The code will display today's date in the output document (well, technically the date the document was last knitted).
...
> >
> > ```{r my-fancy-plot, answer = TRUE, purl = FALSE, eval=FALSE}
> > interviews_plotting %>%
